...
if __name__ == "__main__":
    ...
    # Attempt to create a user under 15
    try:
    ...
    # Lend books
    Loan.lend_book(book_id=book1.id, user_id=user1.id)  # Success
    Loan.lend_book(book_id=book1.id, user_id=user2.id)  # Success
    Loan.lend_book(book_id=book2.id, user_id=user1.id)  # Success
    try:
        Loan.lend_book(book_id=book2.id, user_id=user1.id)  # Error: User max books
    except ValueError as e:
        print(e)
    try:
        Loan.lend_book(book_id=book1.id, user_id=user1.id)  # Error: No copies available
    except ValueError as e:
        print(e)
